...
export function reducer(
  ...
  switch (action.type) {
    ...
    case ReserveActionTypes.LoadReserves: {
      console.debug('reserves読み込み時のアクション(LoadReserves)実行');
      return { ...state, loading: true };
    }

    // reserves読み込み成功時のアクション
    case ReserveActionTypes.LoadReservesSuccess: {
      console.debug('reserves読み込み成功時のアクション(LoadReservesSuccess)実行');
    ...
    case ReserveActionTypes.LoadReservesFail: {
      console.debug('reserves読み込み失敗時のアクション(LoadReservesFail)実行');
